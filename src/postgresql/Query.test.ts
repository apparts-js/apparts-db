...
describe("Pagination", () => {
  ...
  it("Backward compat: findById with positional limit", async () => {
    await expect(
      dbs.collection("testTable").findById({}, 1).toArray()
    ).resolves.toMatchObject([{ id: 1, number: 100 }]);
  });

  it("Backward compat: findById with positional limit and offset", async () => {
    await expect(
      dbs.collection("testTable").findById({}, 1, 1).toArray()
    ).resolves.toMatchObject([{ id: 2, number: 101 }]);
  });

  it("Backward compat: findById with positional null, null, order", async () => {
    await expect(
      dbs
        .collection("testTable")
        .findById({}, null, null, [{ key: "id", dir: "DESC" }])
