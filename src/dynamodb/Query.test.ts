...
runOrSkip("DynamoDB Query filter operators via Scan", () => {
...
runOrSkip("DynamoDB Query unsupported operations", () => {
  let dbs: DBS;
  ...
  test("update with a non-primary-key filter throws NotSupportedByDBEngine", async () => {
    await expect(
      dbs.collection(UNSUP_TABLE).update({ number: 1 }, { tag: "x" })
    ).rejects.toBeInstanceOf(NotSupportedByDBEngine);
  });

  test("remove with a non-primary-key filter throws NotSupportedByDBEngine", async () => {
    await expect(
      dbs.collection(UNSUP_TABLE).remove({ tag: "x" })
    ).rejects.toBeInstanceOf(NotSupportedByDBEngine);
  });
