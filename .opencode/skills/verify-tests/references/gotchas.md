...
Patterns that make tests _look_ correct but silently miss bugs. Scan for all of these on every audit — they recur whenever new tests are added.
...
`toMatchObject` ignores unexpected extra keys — a query leaking extra columns will pass. Use `toStrictEqual` when asserting the _complete_ return shape; reserve `toMatchObject` for checking a subset of fields only.
