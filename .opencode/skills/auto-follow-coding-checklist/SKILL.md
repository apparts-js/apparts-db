...
Your job is to look at open GitHub issues labeled "opencode", figure out what state they are in, and drive the work to completion — creating subtasks in the issue, implementing the code, and opening (or finalizing) a PR.

## Step 1 — Gather open issues labeled "opencode"

```bash
gh issue list --label opencode --state open --json number,title,labels,body,url --limit 50
```
...
```bash
python .opencode/skills/auto-follow-coding-checklist/scripts/classify_issue.py <number>
```
...
```bash
python .opencode/skills/auto-follow-coding-checklist/scripts/check_subtask.py <number> "<exact subtask text>"
```
...
Test coverage audit:
Use the `skill` tool to load the `verify-tests` skill with the commit range as the argument, then follow its instructions.
...
Use the `skill` tool to load the `code-review` skill with the commit range as the argument, then follow its instructions.
...
```bash
python .opencode/skills/auto-follow-coding-checklist/scripts/post_review_comments.py \
...
# once green:
python .opencode/skills/auto-follow-coding-checklist/scripts/check_subtask.py <number> "CI passes and PR is ready for review"
```
...
Use the `skill` tool to load the `resolve-pr-conflicts` skill with the PR number as the argument, then follow its instructions.

---

## State D — Failing CI

Use the `skill` tool to load the `fix-pr-ci` skill with the PR number as the argument, then follow its instructions.
