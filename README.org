...
The database drivers are declared as *optional peer
...
If you select an engine in the config (=use: "postgresql"= or
=use: "dynamodb"=) without its peer dep installed, =require()=
will fail loudly at startup.
...
- ~notin~ :: ~val~ then must be an array of values. The filter
  accepts values that do not appear in the ~val~ array.
